    ...
    for i in range(0,3):
        for j in range(0,3):
                if goal_state[i][j]!=current_state[i][j] and not(i==1 and j==1):
                    ...
                    elif current_state[i][j]==3:
                        temp=abs(0-i)+abs(2-j)

                    elif current_state[i][j]==4:
                        temp=abs(1-i)+abs(2-j)

                    elif current_state[i][j]==5:
                        temp=abs(2-i)+abs(2-j)

                    elif current_state[i][j]==6:
                        temp=abs(2-i)+abs(1-j)

                    elif current_state[i][j]==7:
                        temp=abs(2-i)+abs(0-j)

                    elif current_state[i][j]==8:
                        temp=abs(1-i)+abs(0-j)
 
                    out_place=temp+out_place
	#print("OUT_PLACE",out_place)                
	return out_place
